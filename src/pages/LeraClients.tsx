...
const LeraClients = () => {
  ...
  const [clients, setClients] = useState<LeraClient[]>([]);
  const [editingClient, setEditingClient] = useState<LeraClient | null>(null);
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      if (user.role !== 'admin' && user.role !== 'lera') {
        navigate('/admin/login');
        return;
      }
  ...
  const loadClients = async () => {
    setLoading(true);
    console.log('Loading Lera clients...');
    try {
      const data = await getLeraClients();
  ...
  const handleSaveClient = async (clientData: Omit<LeraClientInsert, 'id' | 'created_at'>) => {
    console.log('Saving client data:', clientData);
    try {
      let result;
      if (editingClient) {
        console.log('Updating existing client:', editingClient.id);
        result = await updateLeraClient(editingClient.id, clientData);
      } else {
        console.log('Creating new client...');
        result = await createLeraClient(clientData);
          ...
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-2xl font-bold text-primary">
                Клиенты Леры - Управление клиентами
              </h1>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Клиенты Леры</CardTitle>
              <Button
...
const ClientEditor = ({
  ...
  client?: LeraClient;
  onSave: (client: Omit<LeraClientInsert, 'id' | 'created_at'>) => void;
  onCancel: () => void;
}) => {
  const [formData, setFormData] = useState<Omit<LeraClientInsert, 'id' | 'created_at'>>(
    client ? {
